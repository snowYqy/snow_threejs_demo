 ...
 * offset是相对于房间中心的偏移量
 */
export const Furniture: React.FC<FurnitureProps> = ({ furniture }) => {
  const { offset, size, color, type } = furniture;
  ...
  switch (type) {
    case 'bed':
      return (
        <group position={[ox, oy + height / 2, oz]}>
          {/* 床垫 */}
          <mesh position={[0, 0, 0]}>
    ...
    case 'fridge':
      return (
        <mesh position={[ox, oy + height / 2, oz]}>
